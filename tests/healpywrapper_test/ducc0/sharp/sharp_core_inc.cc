...
DUCC0_NOINLINE static void alm2map_kernel(s0data_v & DUCC0_RESTRICT d,  // only called in this file 
...
DUCC0_NOINLINE static void calc_alm2map (sharp_job & DUCC0_RESTRICT job, // only called in this file (calls alm2map_kernel)
...
DUCC0_NOINLINE static void calc_map2alm (sharp_job & DUCC0_RESTRICT job, // calls map2alm_kernel
  const sharp_Ylmgen &gen, s0data_v & DUCC0_RESTRICT d, size_t nth)
  {
  size_t l,il=0,lmax=gen.lmax;
  size_t nv2 = (nth+VLEN-1)/VLEN;
...
DUCC0_NOINLINE static void calc_alm2map_spin (sharp_job & DUCC0_RESTRICT job, // calls kernel
...
DUCC0_NOINLINE static void calc_map2alm_spin (sharp_job & DUCC0_RESTRICT job, // calls map2alm_spin_kernel
  const sharp_Ylmgen &gen, sxdata_v & DUCC0_RESTRICT d, size_t nth)
  {
  size_t l,lmax=gen.lmax;
  size_t nv2 = (nth+VLEN-1)/VLEN;
...
DUCC0_NOINLINE static void inner_loop_m2a(sharp_job &job, const vector<bool> &ispair, //only called in this file
  const vector<double> &cth_, const vector<double> &sth_, size_t llim, size_t ulim, //calls calc_map2alm
  sharp_Ylmgen &gen, size_t mi, const vector<size_t> &mlim)
  {
  const size_t m = job.ainfo.mval(mi);
  gen.prepare(m);

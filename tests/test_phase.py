...
print("phase2alm(alm2phase(map))) / map2alm(map)", alm_from_map / geom.map2alm(m, lmax, mmax, 10, [-1, 1]))

map_from_alm = geom.phase2map(phase_from_alm, lmax, mmax, 10, [-1, 1])

print("map from alm: (with phase intermediate)", map_from_alm)
print("phase2map(map2phase(alm))) / alm2map(alm)", map_from_alm / geom.alm2map(alm, lmax, mmax, 10, [-1, 1]))

...
def test_alm2phase2alm_spin():
    ...
    alm_back = hp.sphtfunc.map2alm(
        hp.sphtfunc.alm2map(alm, nside, lmax, pol=True), lmax, iter=0, pol=True
    )

    geom = scarf.healpix_geometry(nside, 1)
    phase_from_alm = geom.alm2phase_spin(alm[1:], 2, lmax, mmax, 10, [-1, 1])
    alm_from_phase = geom.phase2alm_spin(phase_from_alm, 2, lmax, mmax, 10, [-1, 1])

    assert np.linalg.norm(alm_from_phase - alm_back[1:]) < 1e-4


def test_map2phase2map_spin():
    nph = 128
    ntheta = 128
    lmax = 63
    mmax = 63
    ...
    geom = scarf.gauss_geometry(nph, ntheta)
    map = geom.alm2map_spin(
        np.random.random(size=(2, hp.Alm.getsize(lmax, mmax))) * (2 + 1j),
        2,
        lmax,
        mmax,
        1,
...
test_map2phase2map_spin()
test_alm2phase2alm_spin()
